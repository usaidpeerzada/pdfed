/**
 * pdfed - Main Toolbar Component
 * Beautiful floating toolbar for PDF editing
 * 
 * Coordinates all editing tools and manages state
 */
...
const ICONS = {
  ...
  highlight: '<svg viewBox="0 0 24 24"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>',
  ...
  shapes: '<svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/></svg>',
  ...
  signature: '<svg viewBox="0 0 24 24"><path d="M2 17c.64.62 1.61.76 2.37.25.78-.52 1.27-1.37 1.63-2.25.71-1.73 1-3.5 2-5.25.52-.92 1.24-1.75 2.24-2.18 1-.43 2.24-.24 3.01.54.8.8 1 2 .68 3.07-.32 1.07-1 2-1.68 2.82-.68.82-1.5 1.5-2.25 2.25"/><path d="M12 21c1.65 0 3 0 4.5-1.5s1.5-3 1.5-4.5"/><path d="M19 15c1-1 2.5-2.5 2.5-3.5 0-1.65-1.35-3-3-3s-3 1.35-3 3c0 1 1.5 2.5 2.5 3.5"/></svg>',
  stamp: '<svg viewBox="0 0 24 24"><rect x="3" y="17" width="18" height="4" rx="1"/><path d="M6 17V7a4 4 0 0 1 4-4h4a4 4 0 0 1 4 4v10"/></svg>',
  comment: '<svg viewBox="0 0 24 24"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>',
  pages: '<svg viewBox="0 0 24 24"><rect x="8" y="2" width="12" height="18" rx="1"/><path d="M4 6h4"/><path d="M4 10h4"/><path d="M4 14h4"/></svg>',
...
const TOOLS = {
  select: { id: 'select', icon: ICONS.select, label: 'Select', shortcut: 'V', group: 'general' },
  text: { id: 'text', icon: ICONS.text, label: 'Text', shortcut: 'T', group: 'edit' },
  draw: { id: 'draw', icon: ICONS.draw, label: 'Draw', shortcut: 'D', group: 'edit' },
  shapes: { id: 'shapes', icon: ICONS.shapes, label: 'Shapes', shortcut: 'S', group: 'edit' },
  image: { id: 'image', icon: ICONS.image, label: 'Image', shortcut: 'I', group: 'edit' },
  highlight: { id: 'highlight', icon: ICONS.highlight, label: 'Highlight', shortcut: 'H', group: 'annotate' },
  underline: { id: 'underline', icon: ICONS.underline, label: 'Underline', shortcut: 'U', group: 'annotate' },
  strikethrough: { id: 'strikethrough', icon: ICONS.strikethrough, label: 'Strike', shortcut: '', group: 'annotate' },
  comment: { id: 'comment', icon: ICONS.comment, label: 'Comment', shortcut: 'C', group: 'annotate' },
  signature: { id: 'signature', icon: ICONS.signature, label: 'Sign', shortcut: '', group: 'sign' },
  stamp: { id: 'stamp', icon: ICONS.stamp, label: 'Stamp', shortcut: '', group: 'sign' },
  pages: { id: 'pages', icon: ICONS.pages, label: 'Organize', shortcut: 'P', group: 'actions' },
  undo: { id: 'undo', icon: ICONS.undo, label: 'Undo', shortcut: '⌘Z', group: 'actions' },
  redo: { id: 'redo', icon: ICONS.redo, label: 'Redo', shortcut: '⌘⇧Z', group: 'actions' },
  save: { id: 'save', icon: ICONS.save, label: 'Save', shortcut: '⌘S', group: 'actions' }
};
...
const TOOL_GROUPS = [
  ...
  { id: 'annotate', tools: ['highlight', 'underline', 'strikethrough', 'comment'] },
  { id: 'sign', tools: ['signature', 'stamp'] },
  { id: 'actions', tools: ['pages', 'undo', 'redo', 'save'] }
];
...
export class Toolbar {
  ...
  _initializeTools() {
    ...
    const mountPoint = (this.pdfViewer && this.pdfViewer.container) || document.body;
    this.canvasLayer = new CanvasLayer(mountPoint, this._handleAnnotationComplete.bind(this));
  ...
  _selectTool(toolId) {
    // Actions
    switch (toolId) {
      case 'undo': this._handleUndo(); return;
      case 'redo': this._handleRedo(); return;
      case 'save': this._handleSave(); return;
      case 'image': 
    ...
    switch (toolId) {
      ...
      case 'strikethrough':
        return `
          <div class="pdfed-props">
            <label>Color</label>
            <input type="color" value="${toolId === 'highlight' ? '#ffff00' : '#000000'}" id="pdfed-highlight-color" data-option="color">
          </div>
          <div class="pdfed-props">
            <label>Opacity</label>
  ...
  _handleAnnotationComplete(annotation) {
    ...
    // Special Case: Text Start Event
    if (annotation.type === 'text-start') {
        this.textTool.startInput(annotation.x, annotation.y);
        return;
    }

    // Special Case: Text Edit Event (Smart Tool)
    if (annotation.type === 'text-edit') {
        const { bounds, data } = annotation.annotation;
        // Reactivate TextTool with existing content
        // We pass the bounds so input appears exactly over old text
        this.textTool.startInput(bounds.x, bounds.y, {
            text: data.text,
            ...data
        });
        return;
    ...
    // Add to PDF engine (Persist to File)
    if (this.engine) {
      // NOTE: Coordinates from CanvasLayer are screen pixels (clientX/Y) relative to viewport.
      // PDF engine expects PDF coordinates (bottom-up).
      // You will likely need a coordinate conversion step here if PDF is scaled or scrolled.
      // For now, assuming direct mapping or that Engine handles mapping if you pass viewport data.
      
      const { bounds, data, type } = annotation;
      const color = this._hexToRgb(data.color);

      // Simple mapping assuming 1:1 for MVP. 
      // Ideally pass pageNum and converted coords.
      const pageNum = 1; 
      
      switch (type) {
          case 'highlight':
             this.engine.addRectangle(pageNum, bounds.x, bounds.y, bounds.width, bounds.height, {
                color, opacity: data.opacity
             });
             break;
          case 'underline':
          case 'strikethrough':
             // Logic simplified for brevity - Engine expects lines
             break;
          case 'draw':
             // Engine expects points
             break;
      }
    }
  ...
  _handleTextComplete(text, x, y, options, size) {
    console.log('pdfed: Text added:', text, x, y, options);
    
    // 1. Add to PDF Engine (File)
    if (this.engine) {
      const color = this._hexToRgb(options.color);
      // Adjust Y for PDF coordinate system (bottom-up) if needed, 
      // but for MVP let's assume engine handles it or we pass raw.
      this.engine.addText(1, text, x, y, { size: options.fontSize, color });
    }
    
    // 2. Add to Canvas Layer (Visual Persistence) <--- THIS WAS MISSING
if (this.canvasLayer) {
        this.canvasLayer.annotations.push({
            ...
            bounds: { 
                x: x, 
                y: y, // Use the visual Y (top-left) directly
                width: size?.width || 100,  // Use real width
                height: size?.height || 20  // Use real height
            },
            data: { 
                text: text,
                ...options // contains bold, italic, color, backgroundColor
            }
        });
        this.canvasLayer._redraw();
    }

    this.annotationHistory.push({ type: 'text', data: { text, x, y, options } });
  }
  ...
  async _handleSave() {
    ...
    try {
      // Collect Form Data (Interactive Forms)
      let formValues = null;
  ...
  _showSignatureModal() {
    const modal = new SignatureModal((dataUrl) => {
        if (this.canvasLayer) {
            // Calculate center based on container scroll
            const container = (this.pdfViewer && this.pdfViewer.container) || document.documentElement;
            const x = (container.scrollLeft || 0) + (container.clientWidth / 2) - 100;
            const y = (container.scrollTop || 0) + (container.clientHeight / 2) - 50;
  ...
  _addStamp() {
      const modal = new StampModal((dataUrl) => {
          if (this.canvasLayer) {
              // Smart Positioning using Container Scroll
              const container = (this.pdfViewer && this.pdfViewer.container) || document.documentElement;
              const cx = container.clientWidth || window.innerWidth;
              const cy = container.clientHeight || window.innerHeight;
  ...
  _togglePagesDrawer() {
      ...
      host.style.cssText = 'position: absolute; top: 100%; left: 0; right: 0; z-index: -1;';
  ...
  _renderDrawerContent(drawer) {
      ...
      const icons = {
          ...
          rotate: '<svg viewBox="0 0 24 24"><path d="M21 12a9 9 0 1 1-9-9c2.52 0 4.93 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/></svg>',
          ...
          insert: '<svg viewBox="0 0 24 24"><rect x="4" y="2" width="16" height="20" rx="2"/><path d="M12 10v8"/><path d="M8 14h8"/></svg>',
          extract: '<svg viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>'
      };
      ...
      const btns = [
          { label: 'Grid View', icon: icons.grid, action: () => {
               if (!this.pageOrganizer) this.pageOrganizer = new PageOrganizer(this.engine, this._handlePageUpdate.bind(this));
               this.pageOrganizer.open();
               this._togglePagesDrawer(); // Close drawer
          }},
          { label: 'Rotate', icon: icons.rotate, action: () => ops.rotateCurrentPage('right') },
          { label: 'Delete', icon: icons.delete, action: () => ops.deleteCurrentPage() },
          { label: 'Insert', icon: icons.insert, action: () => ops.insertPDF() },
          { label: 'Extract', icon: icons.extract, action: () => ops.extractCurrentPage() }
      ];

      drawer.innerHTML = btns.map((b, i) => `
  ...
  async _handlePageUpdate(newBytes) {
      ...
      // 4. Re-bind Tools
      // CRITICAL: Destroy old CanvasLayer to prevent event listener accumulation
      if (this.canvasLayer) {
  ...
  _handleKeydown(e) {
    ...
    if (isCmd && e.key === 'z' && !e.shiftKey) { e.preventDefault(); this._handleUndo(); }
    else if (isCmd && (e.key === 'y' || (e.key === 'z' && e.shiftKey))) { e.preventDefault(); this._handleRedo(); }
    else if (isCmd && e.key === 's') { e.preventDefault(); this._handleSave(); }
    else if (e.key === 'Escape') { this._selectTool('select'); }
    
    if (!isCmd) {
      switch (e.key.toLowerCase()) {
        case 'v': this._selectTool('select'); break;
        case 't': this._selectTool('text'); break;
        case 'h': this._selectTool('highlight'); break;
        case 'd': this._selectTool('draw'); break;
      }
    }
  }
  ...
  _showCommentPopup(annotation) {
      ...
      popup.innerHTML = `
        ...
        <textarea class="pdfed-comment-textarea" placeholder="Add a comment...">${annotation.data.text || ''}</textarea>
